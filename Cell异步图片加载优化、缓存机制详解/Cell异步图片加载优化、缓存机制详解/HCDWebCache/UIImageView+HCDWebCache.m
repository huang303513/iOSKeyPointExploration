...
-(void)downImageWithString:(NSString *)string placeHolder:(UIImage *)laceHolder complete:(downImageBlock)complete{
    ...
    UIImage *cachedImage = manager.images[picurl];
    if (cachedImage) {//内存中有缓存
        ...
        if (ImgaeData) { // 本地磁盘缓存中有图片
            self.image = [UIImage imageWithData:ImgaeData];
            UIImage *deskImage = [UIImage imageWithData:ImgaeData];
            [manager.images setObject:deskImage forKey:picurl];
            complete(deskImage);
            NSLog(@"磁盘缓存找到了");
        }else{//从网络下载
